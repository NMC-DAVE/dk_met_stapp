...
def draw_observation(data, date_obj, map_region):
    ...
    if 'PRE_Time_0808' in data:
        ...
        title_tp = 'Accumulated precipitation ({}'.format(date_obj.strftime("%Y%m%d08 - "))+'{})'.format((date_obj+dt.timedelta(days=1)).strftime("%Y%m%d08"))
    if 'PRE_Time_2020' in data:
        data['rain'] = pd.cut(data['PRE_Time_2020'], bins=bins, labels=keys)
        data['Rainfall'] = '['+data['Lon'].round(2).astype(str) + ',' + data['Lat'].round(2).astype(str) + ']: ' + \
                       data['PRE_Time_2020'].astype(str)
        data['rain_size'] = data['PRE_Time_2020'] + data['PRE_Time_2020'].mean()
        title_tp = 'Accumulated precipitation ({}'.format((date_obj-dt.timedelta(days=1)).strftime("%Y%m%d20 - "))+'{})'.format(date_obj.strftime("%Y%m%d20")) 
