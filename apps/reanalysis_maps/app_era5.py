...
sys.path.append('.')
import draw_maps_era5 as draw_maps
...
def  main():
    # application title
    st.title("历史天气图分析")
    st.header('——检索1959年1月1日以来的地面观测日值和天气图')
    ...
    # Input data source
    data_source = st.sidebar.selectbox('数据源:', ('CFSR', 'ERA5'), index=1)

    # Input data date
    data_date = st.sidebar.date_input(
        "输入日期:", datetime.date(2020, 7, 1),
        min_value=datetime.date(1959, 1, 1),
    ...
    if st.sidebar.button('绘制天气图'):
        # load data
        data = load_variables(date_obj, map_region=map_region, data_source=data_source)
        ...
        p = Process(target=draw_maps.draw_weather_analysis, args=(date_obj, data, map_region, return_dict, data_source))
        p.start()
        p.join()
...
def read_variable(varname, date_obj, data_source='CFSR'):
    """
    ...
    filepath_era5 = "http://10.28.49.118:8080/thredds/dodsC/ERA5/china/%s/era5.%s.%s.nc"%('%s','%s',yyyymmdd)
    ...
    if data_source == 'CFSR':
        ...
        print(filepath_era5)
        data = xr.open_dataset(filepath_era5%(varname,varname))
        data = data.sel(time=date_obj)
    
    return data


def load_variables(date_obj, map_region=[50, 160, 6, 60], data_source='CFSR'):
    """
    ...
    if data_source=='CFSR':
        ...
        st.info('从10.28.49.118本地服务载入ERA5数据(0.25x0.25度, Waiting ~30s.)')
    
        # construct sub region
        sub_region = {'longitude':slice(map_region[0], map_region[1]),
                      'latitude':slice(map_region[3], map_region[2])}
        ...
        # wind field
        my_bar = st.progress(0)
        data = read_variable('u_component_of_wind', date_obj, data_source)
        subdata['u200'] = data['u'].sel(level=200, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})      ; my_bar.progress(4)
        subdata['u500'] = data['u'].sel(level=500, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})      ; my_bar.progress(8)
        subdata['u700'] = data['u'].sel(level=700, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})      ; my_bar.progress(12)
        subdata['u850'] = data['u'].sel(level=850, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})      ; my_bar.progress(16)
        subdata['u925'] = data['u'].sel(level=950, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})      ; my_bar.progress(20)
        data = read_variable('v_component_of_wind', date_obj, data_source)
        subdata['v200'] = data['v'].sel(level=200, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})      ; my_bar.progress(24)
        subdata['v500'] = data['v'].sel(level=500, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})      ; my_bar.progress(28)
        subdata['v700'] = data['v'].sel(level=700, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})      ; my_bar.progress(32)
        subdata['v850'] = data['v'].sel(level=850, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})      ; my_bar.progress(36)
        subdata['v925'] = data['v'].sel(level=950, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})      ; my_bar.progress(40)
        
        # vertical velocity field
        data = read_variable('vertical_velocity', date_obj, data_source)
        subdata['w700'] = data['w'].sel(level=700, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})      ; my_bar.progress(45)
        
        
        # geopotential height
        data = read_variable('geopotential', date_obj, data_source)
        subdata['gh200'] = data['z'].sel(level=200, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})  ; my_bar.progress(55)
        subdata['gh500'] = data['z'].sel(level=500, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})  ; my_bar.progress(60)
        subdata['gh700'] = data['z'].sel(level=700, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})  ; my_bar.progress(62)
        
        # high temperature
        data = read_variable('temperature', date_obj, data_source)
        subdata['t500'] = data['t'].sel(level=500, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})   ; my_bar.progress(64)
        subdata['t700'] = data['t'].sel(level=700, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})   ; my_bar.progress(66)
        subdata['t850'] = data['t'].sel(level=850, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})   ; my_bar.progress(68)
        subdata['t925'] = data['t'].sel(level=950, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})   ; my_bar.progress(70)
        subdata['t500'] = subdata['t500'].metpy.convert_units('degC')
        subdata['t700'] = subdata['t700'].metpy.convert_units('degC')
        subdata['t850'] = subdata['t850'].metpy.convert_units('degC')
        subdata['t925'] = subdata['t925'].metpy.convert_units('degC')
        ...
        # high moisture field
        data = read_variable('specific_humidity', date_obj, data_source)
        subdata['q700'] = data['q'].sel(level=700, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})   ; my_bar.progress(75)
        subdata['q850'] = data['q'].sel(level=850, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})   ; my_bar.progress(80)
        subdata['q925'] = data['q'].sel(level=950, **sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})   ; my_bar.progress(85)
        
        # mean sea level pressure
        data = read_variable('mslp', date_obj, data_source)
        subdata['mslp'] = data['msl'].sel(**sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})         ; my_bar.progress(90)
        subdata['mslp'] = subdata['mslp'].metpy.convert_units('hPa')
        
        # precipitable water
        data = read_variable('total_column_water', date_obj, data_source)
        subdata['pwat'] = data['tcw'].sel(**sub_region).load().rename({'latitude':'lat', 'longitude':'lon'})         ; my_bar.progress(100)
        #subdata['pwat'].metpy.convert_units('mm')
        subdata['pwat'].attrs['units']='mm'
